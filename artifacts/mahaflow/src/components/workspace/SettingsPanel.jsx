...
import { Bell, Check, Clock3, Cpu, Languages, Save, ShieldCheck, SunMoon, UserRound } from "lucide-react";
...
export const SettingsPanel = ({ role, session, profile, theme, setTheme }) => {
  ...
  return <><SectionHeader eyebrow={`${role.toUpperCase()} · SETTINGS`} title={role === "developer" ? "Advanced settings" : "Settings"} description={role === "developer" ? "Control platform integrations and operational defaults." : "Manage your account and notification preferences."}/><div className="settings-grid">
    ...
    {role === "developer" && <section className="settings-section advanced"><div className="settings-title"><Cpu/><div><h3>YOLO26n worker</h3><p>Connection metadata for your future inference service.</p></div></div><label>Worker endpoint<input type="url" value={settings.yolo_worker_url || ""} onChange={event => setSettings({ ...settings, yolo_worker_url: event.target.value })} data-testid="settings-yolo-url-input" placeholder="https://worker.example.com"/></label><label>Model<input readOnly value="YOLO26n" data-testid="settings-yolo-model-input"/></label><div className="integration-state"><i/><span><b>Awaiting worker</b><small>No detections are simulated. Real telemetry appears after your worker writes readings.</small></span></div></section>}
  </div><div className="settings-actions"><button className="primary-button" data-testid="settings-save-button" disabled={busy} onClick={save}><Save size={17}/>{busy ? "Saving…" : "Save settings"}</button>{message && <span className="success-message" data-testid="settings-message"><Check size={15}/>{message}</span>}</div></>;
};