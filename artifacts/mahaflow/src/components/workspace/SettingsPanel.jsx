...
import { Bell, Check, Clock3, Cpu, Languages, LogOut, RefreshCw, Save, ShieldCheck, SunMoon, UserRound } from "lucide-react";
import { getUserSettings, saveUserSettings, updateProfile } from "@/lib/supabaseData";
import { SectionHeader, formatTime12 } from "@/components/workspace/WorkspaceUI";
...
export const SettingsPanel = ({ role, session, profile, theme, setTheme, onSignOut }) => {
  ...
  return <><SectionHeader eyebrow={`${role.toUpperCase()} · SETTINGS`} title={role === "developer" ? "Settings" : "Settings"} description={role === "developer" ? "Control platform integrations and operational defaults." : "Manage your account and notification preferences."}/><div className="settings-grid">
    ...
    <section className="settings-section"><div className="settings-title"><SunMoon/><div><h3>Experience</h3><p>Language, theme, and alerts.</p></div></div><label><Languages size={14}/>Language<select value={settings.language} onChange={event => setSettings({ ...settings, language: event.target.value })} data-testid="settings-language-select"><option value="en">English</option><option value="hi">हिन्दी</option><option value="mr">मराठी</option></select></label><label>Theme<select value={theme} onChange={event => setTheme(event.target.value)} data-testid="settings-theme-select"><option value="light">Light</option><option value="dark">Dark</option><option value="system">System</option></select></label><button className="setting-toggle" data-testid="settings-notifications-toggle" onClick={() => toggle("notifications_enabled")}><Bell size={16}/><span>Notifications</span><i className={settings.notifications_enabled ? "on" : ""}/></button><button className="setting-toggle" data-testid="settings-crowd-alerts-toggle" onClick={() => toggle("crowd_alerts_enabled")}><ShieldCheck size={16}/><span>Crowd alerts</span><i className={settings.crowd_alerts_enabled ? "on" : ""}/></button></section>
     {role === "authority" && <section className="settings-section"><div className="settings-title"><Clock3/><div><h3>Facility operations</h3><p>Basic authority controls.</p></div></div><label>Opening time<input type="time" value={settings.operating_hours_start || ""} onChange={event => setSettings({ ...settings, operating_hours_start: event.target.value })} data-testid="settings-opening-time-input"/><small className="time-preview">{formatTime12(settings.operating_hours_start)}</small></label><label>Closing time<input type="time" value={settings.operating_hours_end || ""} onChange={event => setSettings({ ...settings, operating_hours_end: event.target.value })} data-testid="settings-closing-time-input"/><small className="time-preview">{formatTime12(settings.operating_hours_end)}</small></label><button className="setting-toggle" data-testid="settings-staff-alerts-toggle" onClick={() => toggle("staff_alerts_enabled")}><Bell size={16}/><span>Staff alerts</span><i className={settings.staff_alerts_enabled ? "on" : ""}/></button></section>}
   ...
   </div><div className="settings-actions"><button className="primary-button" data-testid="settings-save-button" disabled={busy} onClick={save}><Save size={17}/>{busy ? "Saving…" : "Save settings"}</button><button className="outline-button settings-signout" type="button" data-testid="settings-signout-button" onClick={onSignOut}><LogOut size={16}/>Sign out</button>{message && <span className="success-message" data-testid="settings-message"><Check size={15}/>{message}</span>}</div></>;
};